...
description: "Fetch a URL or convert a local file (PDF/DOCX/HTML/etc.) into Markdown using `uvx markitdown`, with optional summarization"
---
...
`markitdown` can fetch URLs by itself; this skill mainly wraps it with convenient save-to-file and optional summarization.
...
- optionally produce a short summary of a long document before deeper work
...
Tip: when using `--summary`, the script will **always** write the full converted Markdown to a temp `.md` file and will **always** print a final "Hint" line with the path (so you can open/inspect the full content).
...
3) run `pi` (no-tools, no-session) with the provider & model configured in `to-markdown.mjs` to summarize using your extra prompt
