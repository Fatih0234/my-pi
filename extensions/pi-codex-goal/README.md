...
## Collecting success criteria before goals

Use `/collect-success-criteria <task>` when a task is goal-worthy but the success criteria need to be discovered before goal creation. It asks the agent to inspect the current repo/task context, produce a Success Criteria Sheet, identify verification evidence and regression constraints, and draft a pasteable `/create-goal` command.

It complements `/create-goal`; it does not replace it. By default, it does not activate a goal unless you explicitly ask it to start or create the goal after collection.

```text
/collect-success-criteria Fix the failing checkout retry test and make sure the fix is verified end to end
```

For reusable criteria artifacts, prefer `docs/specs/<task-slug>.md` for team-visible specs or `.pi/goal-intake/<task-slug>.md` for local Pi workflow scaffolding. Do not store secrets in criteria sheets.
