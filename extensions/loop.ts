...
export default function loopExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		description: "Stop the active loop when the breakout condition is satisfied. Call this when the breakout condition given by the loop prompt is met.",
		promptSnippet: "Signal that the current loop's breakout condition has been satisfied and stop looping.",
		promptGuidelines: [
			"Use signal_loop_success only when a loop breakout condition explicitly described in the loop prompt is satisfied.",
		],
		parameters: Type.Object({
			confirm: Type.Optional(
				Type.Boolean({
					description: "Confirmation that the breakout condition is satisfied",
				}),
			),
		}),
