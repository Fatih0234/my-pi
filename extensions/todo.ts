...
class TodoActionMenuComponent extends Container {
	...
	constructor(
		...
		const options: SelectItem[] = [
			...
			{ value: "githubIssue", label: "github issue", description: "Create GitHub issue from todo" },
			...(closed
...
class TodoDetailOverlayComponent {
	...
	handleInput(keyData: string): void {
		...
		if (keyData === "v" || keyData === "V") {
			if (this.selectMode) {
				...
				// Start cursor at the top of the visible area
				this.cursorLine = this.scrollOffset;
			}
			this.tui.requestRender();
			return;
		}
		...
		if (kb.matches(keyData, "tui.select.down")) {
			if (this.selectMode) {
				...
				this.scrollOffset = Math.min(Math.max(0, this.renderedLineCount - this.viewHeight), this.scrollOffset + 1);
			}
			this.tui.requestRender();
			return;
		}
		if (kb.matches(keyData, "tui.select.pageUp") || matchesKey(keyData, Key.left)) {
			const amount = this.viewHeight;
			if (this.selectMode) {
				this.cursorLine = Math.max(0, this.cursorLine - amount);
		...
		if (kb.matches(keyData, "tui.select.pageDown") || matchesKey(keyData, Key.right)) {
			const amount = this.viewHeight;
			const maxOffset = Math.max(0, this.renderedLineCount - this.viewHeight);
			if (this.selectMode) {
				this.cursorLine = Math.min(Math.max(0, this.renderedLineCount - 1), this.cursorLine + amount);
		...
		if (keyData === "g" || keyData === "G") {
			this.onAction("githubIssue");
			return;
		}
	...
	private buildMetaLine(width: number): string {
		...
		if (this.selectMode) {
			...
			const first = Math.min(this.renderedLineCount, this.scrollOffset + 1);
			const last = Math.min(this.renderedLineCount, this.scrollOffset + this.viewHeight);
			meta = this.theme.fg("muted", `${first}-${last}/${this.renderedLineCount || "?"}`);
		}
		const line =
	...
	private buildActionLine(width: number): string {
		...
		const pieces = this.selectMode
			...
			: [
					...
					this.theme.fg("accent", "v") + this.theme.fg("muted", " select"),
					this.theme.fg("accent", "g") + this.theme.fg("muted", " issue"),
...
async function execGh(
	...
	try {
		const { stdout, stderr } = await execFileAsync("gh", args, { cwd, timeout });
...
async function getGitHubRepoInfo(
	cwd: string,
): Promise<{ owner: string; name: string; url: string } | { error: string }> {
	try {
		const result = await execGh(["repo", "view", "--json", "owner,name,url"], cwd, 10000);
		if (result.exitCode !== 0) {
			...
			return { error: "No GitHub repository detected. Ensure you are in a git repository with a GitHub remote." };
		}
		const parsed = JSON.parse(result.stdout);
		if (!parsed.owner?.login || !parsed.name) {
			return { error: "No GitHub repository detected." };
		}
		return { owner: parsed.owner.login, name: parsed.name, url: parsed.url };
	} catch (e) {
		...
		return { error: `Failed to detect GitHub repository: ${msg}` };
	}
}
		...
		handler: async (args, ctx) => {
			...
			await ctx.ui.custom<void>((tui, theme, keybindings, done) => {
				...
				const handleOverlayResult = async (record: TodoRecord, result: TodoOverlayResult) => {
					...
					if (result.action === "githubIssue") {
						await applyTodoAction(record, "githubIssue");
						setActiveComponent(selector);
						return;
					}
					if (result.comment) {
